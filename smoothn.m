...
%
%   -- Damien Garcia -- 2009/03, revised 2013/12
%   website: <a
...
if isempty(I)
    MaxIter = 500; % default value for MaxIter %change to 200
else
    try
...
%---
% "Spacing" criterion
I = find(strcmpi(varargin,'Spacing'),1);
d = ndims(y{1});
if isempty(I) || isvector(y{1})
    dI = ones(1,d); % spacing increment
else
    try
        dI = varargin{I+1};
    catch %#ok
        error('MATLAB:smoothn:IncorrectSpacing',...
            'A valid spacing must be chosen')
    end
    if ~isnumeric(dI) || length(dI)~=d;
        error('MATLAB:smoothn:IncorrectSpacing',...
            'A valid spacing must be chosen')
    end    
end
