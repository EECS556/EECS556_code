function pg = msTest(imsegs, data, maps, labelclassifier, segclassifier, normalize,labels)
...
for f = 1:numel(imsegs)
    ...
    % store test results (labels)
   
    seg = imsegs(f).segimage;
    visualize_test=zeros(size(seg,1),size(seg,2));
    for i=1:1:imsegs(f).nseg
        [~, seglabel]=max(pg{f}(i,:));
        visualize_test(seg==i)=seglabel;
    end
    label_image = msrcLabel2Img( visualize_test );
    [~,name,ext] = fileparts(imsegs(f).imname);
    if exist('labels')
        for channel = 1:3
        channel1 = label_image(:,:,channel);
        channel1(labels{f}==0) = 0;
        label_image(:,:,channel) = channel1;
        end
        
        
    end
    imwrite(label_image,[test_label_outdir , '/' name '_test' ext]);
end
